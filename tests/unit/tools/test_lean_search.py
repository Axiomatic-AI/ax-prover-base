...
from ax_agent.tools import lean_search
...
class TestRetryLogic:
    ...
    async def test_retry_with_backoff(self, mock_config):
        ...
        with patch("ax_agent.tools.lean_search.asyncio.sleep") as mock_sleep:
...
class TestHTTPRequests:
    ...
    async def test_make_request_success(self, mock_config):
        ...
        with patch("ax_agent.tools.lean_search.get_lean_search_session", return_value=mock_session):
    ...
    async def test_make_request_with_axleansearch_url_auth(self, mock_config):
        ...
        with (
            patch("ax_agent.tools.lean_search.get_lean_search_session", return_value=mock_session),
            patch("ax_agent.utils.google_auth.get_auth_token", return_value="test-token"),
        ):
            result = await lean_search._make_lean_search_request_with_retry(
                "test query", mock_config
            )
    ...
    async def test_make_request_retry_on_429(self, mock_config):
        ...
        with (
            patch("ax_agent.tools.lean_search.get_lean_search_session", return_value=mock_session),
            patch("ax_agent.tools.lean_search.asyncio.sleep", new_callable=AsyncMock),
        ):
            result = await lean_search._make_lean_search_request_with_retry(
                "test query", mock_config
            )
...
class TestSearchFunction:
    ...
    async def test_lean_search_success(self, mock_config):
        ...
        with patch(
            "ax_agent.tools.lean_search._make_lean_search_request_with_retry",
...
class TestToolCreation:
    ...
    async def test_create_tool_success(self, mock_config):
        ...
        with patch("ax_agent.tools.lean_search.warmup_lean_search", new_callable=AsyncMock):
    ...
    async def test_create_tool_warmup_failure(self, mock_config):
        ...
        with patch(
            "ax_agent.tools.lean_search.warmup_lean_search",
            new_callable=AsyncMock,
    ...
    async def test_create_tool_caches_warmup_result(self, mock_config):
        ...
        with patch(
            "ax_agent.tools.lean_search.warmup_lean_search", new_callable=AsyncMock
        ) as mock_warmup:
    ...
    async def test_tool_invocation(self, mock_config):
        ...
        with (
            patch("ax_agent.tools.lean_search.warmup_lean_search", new_callable=AsyncMock),
            patch(
                "ax_agent.tools.lean_search._make_lean_search_request_with_retry",
                new_callable=AsyncMock,
                return_value=mock_response_data,

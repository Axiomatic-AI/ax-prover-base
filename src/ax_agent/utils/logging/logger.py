"""Logging utilities for ax-agent."""
...
def reconfigure_log_level(level: str) -> None:
    """Reconfigure all ax_agent loggers to a new level.

    Call after config is loaded to override the default level
    set during module-level logger creation.
    """
    numeric_level = getattr(logging, level.upper(), logging.INFO)
    for name, logger_ref in logging.Logger.manager.loggerDict.items():
        if name.startswith("ax_agent") and isinstance(logger_ref, logging.Logger):

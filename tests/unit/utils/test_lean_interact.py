...
async def test_aclose_on_unused_server_is_safe(tmp_path):
    """Closing a server that was never started must not raise (it's called on exit-stack cleanup)."""
    server = LeanInteractServer(str(tmp_path), LeanInteractConfig())
    await server.aclose()  # no AttributeError


async def test_async_context_manager_without_run(tmp_path):
...
async def test_run_invokes_underlying_server(tmp_path, monkeypatch):

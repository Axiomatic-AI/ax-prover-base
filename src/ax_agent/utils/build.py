...
if TYPE_CHECKING:
    from ax_agent.models.messages import ProposalMessage
        ...
        async with semaphore:
            ...
            try:
                stdout, stderr = await asyncio.wait_for(
                    process.communicate(), timeout=config.check_file_timeout
                )
            except TimeoutError as e:
                raise LeanBuildTimeout(
                    f"Build timeout (exceeded {config.check_file_timeout} seconds)"
                ) from e
            finally:
    ...
    def __enter__(self) -> "TemporaryProposal":
        ...
        try:
            ...
            if self.proposal.code:
                success = edit_function(self.base_folder, self.location, self.proposal.code)
                if not success:

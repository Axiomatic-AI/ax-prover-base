...
echo ""
echo "Python version:"
docker run --rm ax-agent-runner:local python3

echo ""
echo "Testing StrEnum import:"
docker run --rm ax-agent-runner:local python3 -c "from enum import StrEnum; print('✓ StrEnum works!')"

echo ""
echo "Testing ax-agent installation:"
docker run --rm ax-agent-runner:local ax-agent --version || true

echo ""
echo "Testing ax-agent help:"
docker run --rm ax-agent-runner:local ax-agent --help | head -20

echo ""
echo "==> Success! Images ready:"
echo "  - leanbench-base:local (lean_benchmarks@${BENCH_COMMIT})"
echo "  - ax-agent-runner:local"
echo ""
echo "To run interactively:"
echo "  docker run -it --rm ax-agent-runner:local bash"

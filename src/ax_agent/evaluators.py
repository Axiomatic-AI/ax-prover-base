...
def tool_usage(run: Run, config: ProverConfig) -> dict[str, int]:
    ...
    for field in fields(config):
        if "tool" in field.name:
            available_tools.extend(getattr(config, field.name))
        elif field.name == "researcher" and hasattr(config.researcher, "tools"):
            available_tools.extend(config.researcher.tools)

    if not available_tools:
        logger.warning("The experiment runs without tools")
        return {"key": "tool_usage", "score": 0}

    tool_calls = {f"{tool_name}_tool": 0 for tool_name in available_tools}

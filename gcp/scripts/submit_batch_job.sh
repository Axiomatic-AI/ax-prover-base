...
# Run ax-agent without a command to trigger config parsing and saving, then show help
ax-agent --config "${CONFIG_FILE}" --save-config "${RESOLVED_CONFIG_NAME}" >/dev/null 2>&1
...
# Run ax-agent
set +e
eval "ax-agent --config /tmp/config.yaml --save-config used_config experiment '$AX_DATASET' \
  --folder '$LEAN_FOLDER' \

...
from ..utils.llm import ainvoke_retry_with_structured_output, create_llm, get_reasoning
...
class ProverAgent:
    ...
    async def _create_tools(self) -> list:
        ...
        tools = []
        for tool_config in self.config.proposer_tools:
    ...
    async def _run_tools_and_respond(
        self,
        response: AIMessage,
        messages: list,
        proposer_agent: Callable,
        llm: RunnableRetry,
        extra_message: str | None = None,
    ) -> list:
        """Execute pending tool calls and get LLM's next response."""
        tool_node = ToolNode(self.proposer_tools)
        ...
        new_response = await proposer_agent(invoke_messages, llm)
    ...
    async def _proposer_node(self, state: ProverAgentState, config: RunnableConfig) -> dict:
        ...
        for _ in range(self.runtime_config.max_tool_calling_iterations):
            if not response.tool_calls:
                break
            new_msgs = await self._run_tools_and_respond(
                response,
                context_messages + all_new_messages[:-1],
                proposer_agent,
                llm_with_tools_retry,
            )
            response = new_msgs[-1]
        ...
        if response.tool_calls:
            new_msgs = await self._run_tools_and_respond(
                ...
                proposer_agent,
                self.llm,
                extra_message="NO MORE TOOL CALLS ALLOWED.",
            )
            response = new_msgs[-1]
            all_new_messages += new_msgs
